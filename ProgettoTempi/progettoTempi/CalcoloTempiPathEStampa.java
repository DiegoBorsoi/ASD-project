...
import progettoASD.Graph;
import progettoASD.MyArrayList;

public class CalcoloTempiPathEStampa {
	...
	public static double tempoMedioNetto( int d , double tMin){
		...
		for(int i=0; i<ripTara; i++){
	        ...
	        Graph grafo = progettoASD.ProgettoASD.creaGrafoEdArchi( words, totalAlph);
		...
		for(int i=0; i<ripLordo; i++){
	        ...
	        int longestPath = progettoASD.ProgettoASD.getLongestPath(grafo);
	        ...
	        progettoASD.ProgettoASD.stampaGrafoDot(words, grafo);
